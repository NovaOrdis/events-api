 ...
 *
 *  https://kb.novaordis.com/index.php/Events_OS_Metrics#CpuSoftwareInterruptTime
 *
...
public class CpuSoftwareInterruptTime extends OSMetricDefinitionBase {
    ...
    public CpuSoftwareInterruptTime(PropertyFactory propertyFactory, OSAddress osAddress) {

        super(propertyFactory, osAddress);

        this.TYPE = Float.class;

        this.LABEL = "CPU Software Interrupt Time";

        this.BASE_UNIT = Percentage.getInstance();

        this.DESCRIPTION = "Percentage of total CPU time spent time spent spent servicing software interrupts.";
    ...
    protected InternalMetricReadingContainer parseSourceFileContent
        ...
        if (OSType.LINUX.equals(osType)) {
            ...
            float value = ((CPUStats) pp.getCurrent()).getSoftirqTimePercentage((CPUStats) pp.getPrevious());
    ...
    protected InternalMetricReadingContainer parseCommandOutput(
        ...
        if (OSType.LINUX.equals(osType)) {
            ...
            try {
                ...
                Float f = PercentageArithmetic.parse(si);
